# MetrixND Binary Tuning Assistant

A Streamlit app that automates the model-review process used throughout this
project: upload a MetrixND xlsx export, get back a ranked, **Add: Yes/No** list
...
A `.xlsx` file exported from MetrixND with (at least) these sheets:

- **Data** — columns `Year, Month, <target>, <regressor 1>, <regressor 2>, ...`
- **Coef** — `Variable, Coefficient, StdErr, T-Stat, P-Value, ...` (as MetrixND reports it)
...
This matches the standard MetrixND model export format used for ResUPC,
...
MetrixND's real model can include **AR(1)/MA(1)** autocorrelation correction.
...
test by actually adding it in MetrixND and re-fitting — not as a final
...
trustworthy as MetrixND's own fit.
