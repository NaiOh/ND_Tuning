...
IMPORTANT CAVEAT: ND's real fit can include AR(1)/MA(1) autocorrelation
...
in ND, not a final answer. See `AR_CAVEAT` for the exact wording shown
in the app.
"""
...
AR_CAVEAT = (
    ...
    "not from ND itself. ND's real model may include AR(1)/MA(1) "
...
def normalize_header(value) -> str:
    """ND/Excel often stores one-off event dummy headers (e.g. "Jan-18"
...
def sign_check(coef_df: pd.DataFrame, continuous_cols: list, p_thresh: float = 0.05) -> list:
    ...
    significant NEGATIVE coefficient in the real ND fit — often a sign
...
def existing_binary_removal_candidates(coef_df: pd.DataFrame, p_thresh: float = 0.05) -> pd.DataFrame:
    """Flag currently-included binary/event variables (mBin.* in ND
...
def build_recommendations(
    ...
    for _, row in removal_df.iterrows():
        recs.append(Recommendation(
            ...
            delta_mape=None,
            pvalue=row["PValue"],
            rationale=f"Not significant in the reported ND fit (p={row['PValue']:.3f} > {p_thresh}).",
        ))
        rank += 1
