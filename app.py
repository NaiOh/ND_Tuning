...
st.set_page_config(page_title="ND Binary Tuning Assistant", layout="wide")
...
with st.sidebar:
    ...
    p_thresh = st.slider(
        ...
        help="Existing binaries in the real ND fit with a reported p-value above this are flagged for removal.",
    )
    bic_pref = st.radio(
...
# ---------------------------------------------------------------------------
st.header("1. Current model diagnostics (as reported by ND)")
...
st.caption(
    ...
    "real ND model yet, so there's no reported p-value to read). \"Remove existing\" and "
    "\"Investigate\" rows are read directly from ND's own Coef sheet. See the AR/MA caveat "
...
with tab4:
    st.write(
        ...
        "significant negative coefficient in the real ND fit. Not necessarily wrong, "
